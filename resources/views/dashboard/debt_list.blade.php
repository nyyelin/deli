            ...
            <table class="table table-bordered w-100 dataTable">
              ...
              <tbody>
                    ...
                    @php
                      ...
                      if($row->status == 0){
                        if($row->paystatus == 2 || $row->paystatus == 4){
                          ...
                          if($row->way){
                            if($row->expense){
                              if($row->expense->expense_type_id == 5){
                                $get_amount += $row->expense->amount;
                              }
                            }
                          }

                          $get_amount += $row->delivery_fees;
                          }

                           if($row->way_with_trash){
                            if($row->way_with_trash->status_code == "003"){
                              if($row->paystatus == 4 || $row->paystatus == 1 || $row->paystatus == 2){
                                $get_amount += $row->deposit;
                              }else{
                                $get_amount += 0;
                              }
                            }
                        ...
                        @if($row->status == 1)
                          @if($row->way)
                            @if($row->way->refund_date == null)
                              @php
                                if($row->paystatus == 4 || $row->paystatus == 1 || $row->paystatus == 2){
                                    $alreay_get_amount += $row->delivery_fees;
                                }else{
                                    $alreay_get_amount += 0;
                                }
                              @endphp
                          ...
                          @if($row->status == 0 || $row->status == 1)
                            @if($row->expense)
                              @if($row->expense->expense_type_id == 5 && $row->expense->status == 2)
                                @php
                                  $alreay_get_amount += $row->expense->amount;
                                @endphp
                              @endif
                            @endif
                          @endif
                ...
                <tr>
                  ...
                  <td>{{number_format($get_amount)}} </td>
                  
                  <td>

                    {{-- paid amount --}}

                    @php $expense_amount = 0; @endphp
                    @if(count($pickup->expenses)>0)
                      @foreach($pickup->expenses as $expense)
                        @if($expense->expense_type_id == 1)
                          @php $expense_amount = $expense->amount; @endphp
                  ...
                  <td>{{number_format($alreay_get_amount)}} </td>
                  <td>
                    @if($pickup->expense)
                      @php
                      if($pickup->expense->expense_type_id == 1){
                        $amount = $pay_amount-($pickup->expense->amount+$get_amount+$alreay_get_amount);
                      }else{
                        $amount = $pay_amount-($get_amount);
                      }
                      @endphp
                  ...
                  <td>
                    @if($get_amount > 0 || ($alreay_get_amount+$expense_amount) != $pay_amount)
                    <a href="{{route('debt_detail',$pickup->id)}}" class="btn btn-warning">Detail</a>
                    @else
